...
Plan de construcción por fases y diario de decisiones.

La forma de trabajo, igual en cada paso: (1) plan primero y esperar el visto
bueno, (2) ejecutar, (3) resumen de cambios, (4) verificar contra un criterio de
*done* escrito **antes** de empezar, (5) commit. Pasos pequeños, commits
frecuentes, y no se avanza de fase con el done anterior en rojo.

El proyecto se construyó con Claude Code sobre este documento. La bitácora del
final es lo que se aprendió por el camino, incluidas las veces que el plan
estaba equivocado.
...
Diario de trabajo del proyecto: qué se decidió, por qué, qué se midió antes de
decidirlo y qué salió mal. Se escribe en el momento, no a posteriori, así que
incluye los callejones sin salida y las veces que el plan estaba equivocado.

Formato de cada entrada: **qué pasó → por qué se resolvió así → la regla que
queda o la señal que hay que vigilar**. Está en castellano a propósito: el
README es la vitrina, esto es el cuaderno.

> _A running engineering log in Spanish: decisions, measurements and dead ends,
> written as they happened. The README is the polished version._

### Sobre el arnés de trabajo
...
### Fase 6 — Diseño, README y despliegue
